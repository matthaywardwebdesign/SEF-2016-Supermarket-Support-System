...
module.exports.getPageOfItems = function(number, offset, callback){
   var conn = db.getConnection();
   r.table("items").orderBy("name").slice(offset, offset + number).run(conn , function (err, cursor){
      if (err){
         callback(error.ERROR_DATABASE_ERROR, null);
      } else {
         cursor.toArray(function (err, result){
...
module.exports.getPageOfSearchItems = function(search, number, offset, callback){
   var conn = db.getConnection();
   r.table("items").filter(function(doc){
    return doc('name').match(search)
}).orderBy("name").slice(offset, offset + number).run(conn , function (err, cursor){
      if (err){
         callback(error.ERROR_DATABASE_ERROR, null);
      } else {
         cursor.toArray(function (err, result){
