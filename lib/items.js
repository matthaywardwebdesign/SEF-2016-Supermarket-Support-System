...
module.exports.getPageOfItems = function(number, offset, sort, callback){
   var conn = db.getConnection();
   r.table("items").orderBy(sort).slice(offset, offset + number).run(conn , function (err, cursor){
      if (err){
         callback(error.ERROR_DATABASE_ERROR, null);
      } else {
         cursor.toArray(function (err, result){
...
module.exports.getPageOfSearchItems = function(search, number, offset, sort, callback){
   var conn = db.getConnection();
   r.table("items").filter(function(doc){
    return doc('name').match("(?i)" + search)
}).orderBy(sort).slice(offset, offset + number).run(conn , function (err, cursor){
      if (err){
         callback(error.ERROR_DATABASE_ERROR, null);
      } else {
         cursor.toArray(function (err, result){
            if (err){
               ...
               if (items.length == 0){
                  module.exports.getItemByID(parseInt(search), function (err, item){
                     if (!err && item != null){
                        items.push(item);
                     }
...
module.exports.deleteItem = function(item, callback){
   var conn = db.getConnection();
   r.table("items").get(item.getID()).delete().run(conn, function (err){
      if (err){
         callback(error.ERROR_DATABASE_ERROR);
      } else {
         callback(null);
...
module.exports.deleteItemByID = function(id, callback){
   var conn = db.getConnection();
   r.table("items").get(id).delete().run(conn, function (err){
      if (err){
         callback(error.ERROR_DATABASE_ERROR);
      } else {
         callback(null);
