...
var Transaction = function(details){
   ...
   self.loyaltyPoints = details.loyaltyPoints || 0;
   self.loyaltyDiscount = details.loyaltyDiscount || 0;
   ...
   Transaction.prototype.pay = function(paymentType, callback){
      ...
      self.calculate(function(){
         ...
         customers.getCustomerByID(self.customerNo, function (err, customer){
            var currentLoyaltyPoints = customer.getLoyaltyPoints();
            customer.setLoyaltyPoints(parseInt(currentLoyaltyPoints)+parseInt(self.loyaltyPoints));
            customers.saveCustomer(customer, function (err){});
         });
         

         //Update item soh and do reorder
         self.getItems(function (data){
            orders.getOpenOrder(function (order){
               for (var i = 0; i < data.length; i++){
                  var item = new Item(data[i].item);
                  item.removeStock(data[i].qty);
                  item.attemptReorder(order);
            ...
            (function(i){
               ...
               self.getSpecialsForTransactionLine(self.items[i], function (specials){
                  ...
                  if (remaining == 0){
                     ...
                     self.loyaltyPoints = Math.floor(self.getTotal()/10);
                     ...
                     customers.getCustomerByID(self.customerNo, function (err, customer){
                        var currentLoyaltyPoints = customer.getLoyaltyPoints();
                        var loyaltyDiscount = Math.floor(currentLoyaltyPoints/20) * 5;
                        if (self.getTotal() < loyaltyDiscount){
                           loyaltyDiscount = Math.floor(self.getTotal() / 5) * 5;
                        }
                        self.loyaltyDiscount = loyaltyDiscount;
                        self.setTotal(self.getTotal() - loyaltyDiscount);
                        self.setGSTApplicableTotal(self.getGSTApplicableTotal() - loyaltyDiscount);
                        self.setGST(self.getGSTApplicableTotal() * 0.10);
